 ...
 *
 * 가격은 예시 기본값으로 두고,
 * 실제 서버 시세에 맞게 사용자가 수정해서 쓸 수 있게 한다.
 */
const INITIAL_FORM = {
  ...
  ownedRareScroll: 0,
  ownedProtectionCharm: 0,
  ownedMoonAuraPotion: 0,
...
function createInitialInput(): EnhancementCalculationInput {
  return {
    ...
    owned: {
      commonScroll: INITIAL_FORM.ownedCommonScroll,
      uncommonScroll: INITIAL_FORM.ownedUncommonScroll,
      rareScroll: INITIAL_FORM.ownedRareScroll,
      protectionCharm: INITIAL_FORM.ownedProtectionCharm,
      moonAuraPotion: INITIAL_FORM.ownedMoonAuraPotion,
    },
    strategy: {
...
export default function EnhancementCalculatorPage() {
   ...
   * 보유 재료 입력값
   *
   * 결과에서 기대 필요량만큼 먼저 차감한 뒤
   * 부족한 양에 대해서만 추가 구매 비용을 계산한다.
   */
  const [ownedCommonScroll, setOwnedCommonScroll] = useState(
    INITIAL_FORM.ownedCommonScroll,
  );
  const [ownedUncommonScroll, setOwnedUncommonScroll] = useState(
    INITIAL_FORM.ownedUncommonScroll,
  );
  const [ownedRareScroll, setOwnedRareScroll] = useState(
    INITIAL_FORM.ownedRareScroll,
  );
  const [ownedProtectionCharm, setOwnedProtectionCharm] = useState(
    INITIAL_FORM.ownedProtectionCharm,
  );
  const [ownedMoonAuraPotion, setOwnedMoonAuraPotion] = useState(
    INITIAL_FORM.ownedMoonAuraPotion,
  );

  /**
   * 단계별 주문서 전략
   * level n 값은 n강 -> n+1강 시도 주문서
   */
  ...
  const buildInput = (): EnhancementCalculationInput => {
    return {
      ...
      owned: {
        commonScroll: ownedCommonScroll,
        uncommonScroll: ownedUncommonScroll,
        rareScroll: ownedRareScroll,
        protectionCharm: ownedProtectionCharm,
        moonAuraPotion: ownedMoonAuraPotion,
      },
      strategy: {
        0: strategy0,
        1: strategy1,
  ...
  const handleReset = () => {
    ...
    setOwnedCommonScroll(INITIAL_FORM.ownedCommonScroll);
    setOwnedUncommonScroll(INITIAL_FORM.ownedUncommonScroll);
    setOwnedRareScroll(INITIAL_FORM.ownedRareScroll);
    setOwnedProtectionCharm(INITIAL_FORM.ownedProtectionCharm);
    setOwnedMoonAuraPotion(INITIAL_FORM.ownedMoonAuraPotion);
    ...
    <>
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-700">
                  ...
                  <div className="mt-2 space-y-1 leading-6">
                    <p>
                      일반 주문서: {commonInfo.minLevel}~{commonInfo.maxLevel}강
                      사용, 성공 시 스탯 +{commonInfo.statIncreaseOnSuccess}, 실패 시
                      기운 -{commonInfo.moonAuraLossOnFail}
                    </p>
                    <p>
                      고급 주문서: {uncommonInfo.minLevel}~
                      {uncommonInfo.maxLevel}강 사용, 성공 시 스탯 +
                      {uncommonInfo.statIncreaseOnSuccess}, 실패 시 기운 -
                      {uncommonInfo.moonAuraLossOnFail}
                    </p>
                    <p>
            ...
            <CalculatorPanel title="보유 재료">
              <div className="space-y-3">
                <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">
                  <p className="font-semibold">보유량 차감 방식</p>
                  <p className="mt-1">
                    이미 가지고 있는 주문서/부적/농축액은 기대 필요량에서 먼저 차감하고,
                    부족한 양만 추가 구매 비용으로 계산합니다.
                  </p>
                </div>

                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="보유 일반 주문서">
                    <NumberInput
                      value={ownedCommonScroll}
                      min={0}
                      onChange={(value) => {
                        setOwnedCommonScroll(value);
                        setIsDirty(true);
                      }}
                    />
                  </Field>

                  <Field label="보유 고급 주문서">
                    <NumberInput
                      value={ownedUncommonScroll}
                      min={0}
                      onChange={(value) => {
                        setOwnedUncommonScroll(value);
                        setIsDirty(true);
                      }}
                    />
                  </Field>

                  <Field label="보유 희귀 주문서">
                    <NumberInput
                      value={ownedRareScroll}
                      min={0}
                      onChange={(value) => {
                        setOwnedRareScroll(value);
                        setIsDirty(true);
                      }}
                    />
                  </Field>

                  <Field label="보유 달빛 부적">
                    <NumberInput
                      value={ownedProtectionCharm}
                      min={0}
                      onChange={(value) => {
                        setOwnedProtectionCharm(value);
                        setIsDirty(true);
                      }}
                    />
                  </Field>

                  <Field label="보유 달빛 기운 농축액">
                    <NumberInput
                      value={ownedMoonAuraPotion}
                      min={0}
                      onChange={(value) => {
                        setOwnedMoonAuraPotion(value);
                        setIsDirty(true);
                      }}
                    />
                  </Field>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                  {visibleLevels.length === 0 ? (
                    <p className="text-sm text-zinc-600">
                      현재 강화 단계가 목표 이상이므로 추가 확률 정보가 없습니다.
                    </p>
                  ) : (
              ...
              <ResultCard title="전체 기대 비용">
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span>보유 재료 차감 전 총 기대 비용</span>
                    <span className="font-semibold text-zinc-900">
                      {formatCell(result.summary.totalExpectedCost)}셀
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>보유 재료 차감 후 추가 구매 비용</span>
                    <span className="font-semibold text-blue-700">
                      {formatCell(result.ownedAdjusted.totalAdditionalPurchaseCost)}셀
                    </span>
                  </div>
                </div>

                <div className="mt-4 flex flex-wrap gap-3">
                  <ActionButton onClick={() => setIsDetailModalOpen(true)}>
                    단계별 상세 보기
                  </ActionButton>
                </div>
              </ResultCard>

              <ResultCard title="보유 재료 차감 결과">
                <div className="space-y-3">
                  <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3">
                    <div className="flex justify-between text-sm">
                      <span className="font-medium text-zinc-900">일반 주문서</span>
                      <span className="text-zinc-700">
                        추가 구매 비용 {formatCell(result.materials.commonScroll.additionalCost)}셀
                      </span>
                    </div>
                    <div className="mt-2 grid gap-1 text-sm text-zinc-700">
                      <p>기대 필요량: {formatInteger(result.materials.commonScroll.expectedNeeded)}개</p>
                      <p>보유량: {formatInteger(result.materials.commonScroll.owned)}개</p>
                      <p>추가 필요량: {formatInteger(result.materials.commonScroll.additionalNeeded)}개</p>
                    </div>
                  </div>

                  <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3">
                    <div className="flex justify-between text-sm">
                      <span className="font-medium text-zinc-900">고급 주문서</span>
                      <span className="text-zinc-700">
                        추가 구매 비용 {formatCell(result.materials.uncommonScroll.additionalCost)}셀
                      </span>
                    </div>
                    <div className="mt-2 grid gap-1 text-sm text-zinc-700">
                      <p>기대 필요량: {formatInteger(result.materials.uncommonScroll.expectedNeeded)}개</p>
                      <p>보유량: {formatInteger(result.materials.uncommonScroll.owned)}개</p>
                      <p>추가 필요량: {formatInteger(result.materials.uncommonScroll.additionalNeeded)}개</p>
                    </div>
                  </div>

                  <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3">
                    <div className="flex justify-between text-sm">
                      <span className="font-medium text-zinc-900">희귀 주문서</span>
                      <span className="text-zinc-700">
                        추가 구매 비용 {formatCell(result.materials.rareScroll.additionalCost)}셀
                      </span>
                    </div>
                    <div className="mt-2 grid gap-1 text-sm text-zinc-700">
                      <p>기대 필요량: {formatInteger(result.materials.rareScroll.expectedNeeded)}개</p>
                      <p>보유량: {formatInteger(result.materials.rareScroll.owned)}개</p>
                      <p>추가 필요량: {formatInteger(result.materials.rareScroll.additionalNeeded)}개</p>
                    </div>
                  </div>

                  <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3">
                    <div className="flex justify-between text-sm">
                      <span className="font-medium text-zinc-900">달빛 부적</span>
                      <span className="text-zinc-700">
                        추가 구매 비용 {formatCell(result.materials.protectionCharm.additionalCost)}셀
                      </span>
                    </div>
                    <div className="mt-2 grid gap-1 text-sm text-zinc-700">
                      <p>기대 필요량: {formatInteger(result.materials.protectionCharm.expectedNeeded)}개</p>
                      <p>보유량: {formatInteger(result.materials.protectionCharm.owned)}개</p>
                      <p>추가 필요량: {formatInteger(result.materials.protectionCharm.additionalNeeded)}개</p>
                    </div>
                  </div>

                  <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3">
                    <div className="flex justify-between text-sm">
                      <span className="font-medium text-zinc-900">달빛 기운 농축액</span>
                      <span className="text-zinc-700">
                        추가 구매 비용 {formatCell(result.materials.moonAuraPotion.additionalCost)}셀
                      </span>
                    </div>
                    <div className="mt-2 grid gap-1 text-sm text-zinc-700">
                      <p>기대 필요량: {formatInteger(result.materials.moonAuraPotion.expectedNeeded)}개</p>
                      <p>보유량: {formatInteger(result.materials.moonAuraPotion.owned)}개</p>
                      <p>추가 필요량: {formatInteger(result.materials.moonAuraPotion.additionalNeeded)}개</p>
                    </div>
                  </div>
                </div>
              </ResultCard>
      ...
      {isDetailModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-6">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-2xl border border-zinc-700 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-4">
              <div>
                <h2 className="text-xl font-bold text-zinc-900">단계별 상세 결과</h2>
                <p className="mt-1 text-sm text-zinc-600">
                  각 강화 단계의 기대 비용 / 기대 시도 횟수 / 주문서 전략을 확인합니다.
                </p>
              </div>
              <button
                type="button"
              ...
              <div className="space-y-4">
                {result.steps.length === 0 ? (
                  <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-4 text-sm text-zinc-700">
                    현재 강화 단계가 목표 이상이므로 추가 상세 결과가 없습니다.
                  </div>
                ) : (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-zinc-600">
                            사용 주문서: {getEnhancementScrollInfo(step.scrollType).label}
                          </p>
                        </div>
                        <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-900">
                          단계 기대 비용 {formatCell(step.expectedCostToClearStep)}셀
                        </div>
                      </div>
                      ...
                      <div className="mt-4 grid gap-3 md:grid-cols-2">
                        <div className="space-y-2 text-sm text-zinc-700">
                          ...
                          <div className="flex justify-between">
                            <span>실패 확률</span>
                            <span>{formatPercentFromRatio(step.failRate, 2)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>실패 시 하락 확률</span>
                          ...
                          <div className="flex justify-between">
                            <span>시도당 실제 하락 확률</span>
                            <span>
                              {formatPercentFromRatio(
                                step.actualDropRatePerAttempt,
                                2,
                              )}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span>시도당 유지 확률</span>
                            <span>
                              {formatPercentFromRatio(step.stayRatePerAttempt, 2)}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2 text-sm text-zinc-700">
                          <div className="flex justify-between">
                            <span>주문서 1회 비용</span>
                            <span>{formatCell(step.scrollCost)}셀</span>
                          </div>
                          <div className="flex justify-between">
                            <span>실패 시 달빛 기운 감소</span>
                            <span>{formatInteger(step.moonAuraLossOnFail)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>시도당 기대 기운 복구비</span>
                            <span>
                              {formatCell(step.expectedMoonAuraRecoveryCostPerAttempt)}셀
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span>시도당 기대 부적 비용</span>
                            <span>
                              {formatCell(step.expectedProtectionCostPerAttempt)}셀
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span>시도당 총 기대비용</span>
                            <span>{formatCell(step.expectedAttemptCost)}셀</span>
                          </div>
                          <div className="flex justify-between">
                            <span>단계 기대 시도 횟수(근사)</span>
                            <span>{formatInteger(step.expectedAttemptsToClearStep)}회</span>
                          </div>
                        </div>
                      </div>
                    </div>
